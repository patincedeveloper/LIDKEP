...
import { submitProfileForReview } from '../services/users.service.js';
...
export async function updateProfile(req, res) {
  await prisma.$transaction(async (tx) => {
    await usersRepository.updateProfile(req.user.id, {
      ...
      privatePhone: req.validated.body.phoneNumber.startsWith('0') ? `+250${req.validated.body.phoneNumber.slice(1)}` : req.validated.body.phoneNumber,
...
export async function notifications(req, res) {
  ...
  res.json(successResponse(req, items.map((item) => ({ id: item.id, title: item.title, message: item.message, time: item.createdAt.toISOString(), read: Boolean(item.readAt), type: item.type }))));
}
